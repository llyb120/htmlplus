...
- ⚡ **轻量高效**: 核心代码 < 50KB，无依赖，开箱即用
- 🔄 **细粒度响应式**: 基于 Proxy 的响应式系统，自动追踪依赖，精准更新
- 🧩 **Web Components**: 基于标准 Web Components，天然支持组件化
- 📦 **双语法支持**: 支持模板字符串 `html``` 和 JSX 两种写法
- 🎨 **插槽支持**: 支持 Light DOM 插槽，方便组件组合
- 🔀 **批量更新**: 自动批处理状态更新，减少 DOM 操作
- 🪝 **生命周期钩子**: 提供 `onMounted`、`onUnmounted` 生命周期管理
- 🎭 **智能 Diff**: 内置列表 Diff 算法，最小化 DOM 操作
